use custos::{CDatatype, Buffer, cuda::{CudaCache, launch_kernel1d}, CudaDevice};
...
/// }
/// ```
pub fn cu_ew<T: CDatatype>(device: &CudaDevice, lhs: &Buffer<T>, rhs: &Buffer<T>, op: &str) -> custos::Result<Buffer<T>> {
    let src = format!(
...
/// }
/// ```
pub fn cu_ew_self<T: CDatatype>(device: &CudaDevice, lhs: &mut Buffer<T>, rhs: &Buffer<T>, op: &str) -> custos::Result<()> {
    let src = format!(
