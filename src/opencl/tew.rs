use custos::{CDatatype, CLDevice, Buffer, opencl::KernelOptions};
...
/// }
/// ```
pub fn cl_tew<T: CDatatype>(device: &CLDevice, lhs: &Buffer<T>, rhs: &Buffer<T>, op: &str) -> custos::Result<Buffer<T>> {
    let src = format!("
...
/// }
/// ```
pub fn cl_tew_self<T: CDatatype>(device: &CLDevice, lhs: &mut Buffer<T>, rhs: &Buffer<T>, op: &str) -> custos::Result<()> {
    let src = format!("
